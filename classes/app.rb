...
class App
  def initialize
    ...
    @book = []
    @rentals = []
    @people = HandleData.new().read_data('person.json')
    @book = HandleData.new().read_data('books.json')
  end
  ...
  def create_student
    ...
    @people << student
    @store.store_people('person.json', {type: '[Student]', name: student.name, age: student.age, parent_permission: permisson, id: student.id })
  ...
  def create_teacher
    ...
    @people << teacher
    @store.store_people('person.json', {type: '[Teacher]', name: teacher.name, age: teacher.age, id: teacher.id })
  ...
  def create_rental
    ...
    if @book.empty?
      ...
      @rentals << rental
      # @store.store_rentals('rentals.json', {date: rental.date, person: @person[person_index], book: @book[book_index]})
      puts 'Rental created successfully'
    end
  end

  def list_rentals
    @rentals = HandleData.new().read_data('rentals.json')
    print 'ID of person: '
    id = gets.chomp.to_i
